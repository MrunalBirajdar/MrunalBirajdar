	...
	if(isset ($_POST['submit'])){
		...
		$imagepath=$_FILES['image']['tmp_name'];
		 ...
		 if($imagepath){
		 	...
		 	$update=mysqli_query($conn,"UPDATE mrunal1 SET title='$title',image='$picture',content='$content',datep='$datep' WHERE id='$id'");
		 	if($update){
		 		echo"good";
		 	...
		 	$update=mysqli_query($conn,"UPDATE mrunal1 SET title='$title',content='$content',datep='$datep' WHERE id='$id'");
		 	if($update){
		 		echo"good";
		 		echo "<script language='Javascript'>";
		 		echo "document.location.replace('./page.php')";
